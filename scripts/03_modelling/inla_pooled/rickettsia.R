...
rseed = 3745068
disease_name = "rickettsia"

# disease data and shapefile post-1985
dz = read.csv("./output/spillovers_processed/spillovers_rickettsia.csv") %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>%
  dplyr::filter(!is.na(Longitude)) 
load("./output/spillovers_processed/spillovers_rickettsia_shp.R")
dz_shp = shp %>%
  dplyr::filter(ADMcode %in% dz$ADMcode)

# create study area polygon
bounds = sf::st_read("./data/spillovers/Rickettsioses/gadm41_BRA_0.shp")
