...
filter_area_thresh = 20000 # maximum geographical area of occurrence polygons to include in km2 (i.e. exclude v imprecise records)
num_background_points = 15000 # number of background points to generate
...
if(generate_dataset == TRUE){
  ...
  # covars for background points only (read in US admin covars separately)
  dd = bg %>% dplyr::select(record_id, Longitude, Latitude, ADMcode) %>% dplyr::mutate(presence = 0)
  
  # extract covariates for background points
  ...
  # create full df
  # read in background points and combine with presences
  ...
  dzp = dz %>%
    ...
    st_drop_geometry() %>%
    dplyr::left_join(
      read.csv("./output/model_df/usa_admin2_covars.csv") %>% dplyr::select(-Admin_name)
    ) %>%
    dplyr::mutate(health_travel_log = log(health_travel+1),
                  livestock_log = log(livestock_all+1))
  ...
  # combine and save
  dd = rbind(dd, dzp)
  filename = paste("./output/model_df/" , disease_name, "_df_wp_full.csv", sep="")
...
filename = paste("./output/model_df/" , disease_name, "_df_wp_full.csv", sep="")
dd = read.csv(filename) %>%
  dplyr::select(-health_travel, -livestock_all, -tmean, -precip, -tmean_anomaly, -precip_anomaly)
...
mc_vars_to_exclude = c("social_vulnerability", "health_travel_log")
