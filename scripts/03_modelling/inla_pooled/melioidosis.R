...
dz = read.csv("./output/spillovers_processed/spillovers_melioidosis.csv") %>%
  dplyr::filter(Year >= 1985) %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>%
  dplyr::filter(Longitude > 60 & Longitude < 155)
...
num_background_points = round(nrow(dz) * 2.5) # number of background points to generate
