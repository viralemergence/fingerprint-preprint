...
dz = read.csv("output/spillovers_processed/spillovers_zika.csv") %>%
  dplyr::filter(Longitude < -25 & Longitude > -100) %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>%
  dplyr::filter(!is.na(Longitude)) %>%
  dplyr::filter(Year >= 1985)
# n.b. 5 points removed due to data sparsity and absences issue
load("./output/spillovers_processed/spillovers_zika_shp.R")
dz_shp = shp1 %>%
  dplyr::filter(ADMcode %in% dz$ADMcode)

# create study area polygon
study_area = createStudyAreaPolygon(lat=dz$Latitude, lon=dz$Longitude, buffer = TRUE, extent_border = 5, buf_size=180000)
...
mc_vars_to_exclude = c("urban_expansion", "social_vulnerability")
