...
rseed = 68402
disease_name = "pknowlesi"

# disease data and shapefile post-1985
dz = read.csv("./output/spillovers_processed/spillovers_pknowlesi.csv") %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>%
  dplyr::filter(!is.na(Longitude)) %>%
  dplyr::filter(Year >= 1985)
load("./output/spillovers_processed/spillovers_pknowlesi_shp.R")
dz_shp = shp %>%
...
mc_vars_to_exclude = c("urban_expansion", "urban_cover", "forest_cover", "hunting", "social_vulnerability", "biodiv_intact", nas)
