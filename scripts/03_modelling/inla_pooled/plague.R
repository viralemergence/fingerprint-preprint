...
dz = read.csv("./output/spillovers_processed/spillovers_plague.csv") %>%
  dplyr::filter(Year >= 1985) %>%
  dplyr::filter(Country == "USA") %>%
  distinct() %>%
  dplyr::filter(!is.na(Longitude)) %>%
  dplyr::mutate(record_id = 1:length(Disease))
load("./output/spillovers_processed/spillovers_plague_shp.R")
dz_shp = shp1 %>% dplyr::mutate(ADMcode = as.numeric(ADMcode)) %>% dplyr::filter(ADMcode %in% dz$ADMcode); rm(shp1)
...
mc_vars_to_exclude = c("health_travel_log", "social_vulnerability", "forest_cover", "crop_expansion", "livestock_log")
