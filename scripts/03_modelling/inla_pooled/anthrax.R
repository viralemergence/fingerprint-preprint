...
# setup objects
# PATH = dirname(dirname(dirname(dirname(rstudioapi::getSourceEditorContext()$path))))
# setwd(PATH)
# library(raster); library(dplyr); library(magrittr); library(ggplot2); 
# library(sf); library(reticulate); library(rgee)
...
dz = read.csv("./output/spillovers_processed/spillovers_anthrax.csv") %>%
  dplyr::filter(Year >= 1980) %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease)) 

# create study area polygon
study_area = createStudyAreaPolygon(lat=dz$Latitude, lon=dz$Longitude, buffer = TRUE)
...
num_background_points = nrow(dz) * 8 # number of background points to generate
...
hyp_lib = read.csv("./output/hypotheses/consensus_hypotheses_matched.csv") %>%
  ...
  dplyr::filter(Disease == dz$Disease[1]) %>%
