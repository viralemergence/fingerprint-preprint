...
dz = read.csv("./output/spillovers_processed/spillovers_henipa.csv") %>%
  dplyr::filter(Disease == "Nipah virus disease") %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>% 
  dplyr::mutate(dup = duplicated(paste(LocationName, Year))) %>%
  dplyr::filter(dup == FALSE) %>%
  dplyr::select(-dup)
load("./output/spillovers_processed/spillovers_henipa_shp.R")
dz_shp = shp1 %>% dplyr::filter(ADMcode %in% dz$ADMcode); rm(shp1)
...
mc_vars_to_exclude = c("urban_expansion", "crop_cover", "biodiv_intact")
...
# filter out some from strict hyp bc insufficient data
hyp_strict = hyp_strict[ -which(hyp_strict %in% c("urban_cover", "social_vulnerability"))]
