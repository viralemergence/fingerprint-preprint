...
filter_area_thresh = 8000 # maximum geographical area of occurrence polygons to include in km2 (i.e. exclude v imprecise records)
...
dz = read.csv("C:/Users/roryj/Documents/Research/projects_current/202011_fingerprint/arbonet/fingerprint_formatted/spillovers_arbonet.csv") %>%
  dplyr::filter(Disease == "LaCrosse encephalitis" & Presentation == "All cases") %>%
...
hyp_strict = hyp_strict[ ! hyp_strict %in% c("urban_expansion", "biodiv_intact", "tmean_change", "health_travel_log") ]
...
df = expand.grid(shp$ADMcode, unique(dz$Year)) %>%
  as.data.frame() %>%
  dplyr::rename(
    "ADMcode"=1, "Year"=2
  ) %>%
...
save(m1, file="./output/model_outputs/wnv_testmodels/lacrosse_spde_mod1.R")
...
save(m2, file="./output/model_outputs/wnv_testmodels/lacrosse_spde_mod2.R")
...
load(file="./output/model_outputs/wnv_testmodels/lacrosse_spde_mod1.R")
load(file="./output/model_outputs/wnv_testmodels/lacrosse_spde_mod2.R")
