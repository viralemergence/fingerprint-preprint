...
occurrence_point_buffer_km = 5 # km buffer around each occurrence point for spatial uncertainty in spillover event
...
num_background_points = 9000 # number of background points to generate
...
# disease data and shapefile post-2004 (after expansion in USA)
disease_name = "wnv"
...
covars = covars %>% dplyr::select(-urban_expansion, -health_travel_log)
hyp_strict = hyp_strict[ ! hyp_strict %in% c("urban_expansion", "health_travel_log") ]
...
m1 = fitINLAModel(formx = formula(form), family="nbinomial", stack=stack1, spde=spde, inla.mode="experimental")
save(m1, file="./output/model_outputs/wnv_testmodels/wnv_spde_mod1.R")
...
# ------------ MODEL 2: Annual presence + pseudoabsences ----------
...
load(file="./output/model_outputs/wnv_testmodels/wnv_spde_mod1.R")
load(file="./output/model_outputs/wnv_testmodels/wnv_spde_mod2.R")
