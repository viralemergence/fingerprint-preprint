...
# -------- select polygon pseudoabsences (n=1000) ------------

npts = 1000
shp_pseudo = sf::st_read("C:/Users/roryj/Documents/PhD/202011_fingerprint/arbonet/arbonet_countyarboviral/cb_2018_us_county_500k.shp") %>%
  dplyr::mutate(CTYCODE = as.integer(COUNTYFP),
                STCODE = as.integer(STATEFP),
                ADMcode = paste(STATEFP, COUNTYFP, sep="")) %>% 
  st_transform(crs = 4326) 
shp_pseudo = shp_pseudo[ study_area, ]
shp_pseudo = shp_pseudo[  sample(1:nrow(shp_pseudo), npts, replace=TRUE), ]
shp_pseudo$record_id = paste("pa", 1:npts, sep="_")

shp_pseudo = cbind(shp_pseudo, st_coordinates(st_centroid(shp_pseudo)) %>% as.data.frame %>% dplyr::rename("Longitude"=1, "Latitude"=2))

# jitter lat-lons for spatial model
# locs_jit = data.frame()
# for(i in 1:nrow(shp_pseudo)){
#   cat(i)
#   locs_jit = rbind(locs_jit, st_coordinates(sf::st_sample(shp_pseudo[i, ], size=1)))
# }
# shp_pseudo = cbind(shp_pseudo, locs_jit)
# shp_pseudo$Longitude = shp_pseudo$X
# shp_pseudo$Latitude = shp_pseudo$Y
...
dd = rbind(
  ...
  shp_pseudo %>% dplyr::select(record_id, Longitude, Latitude) %>% dplyr::mutate(presence = 0)
)

pl = ggplot() + 
  geom_sf(data=study_area, fill="grey97") + 
  maptheme +
  geom_point(data=as.data.frame(shp_pseudo), aes(Longitude, Latitude), color="coral", size=0.4) +
  geom_point(data=as.data.frame(dz), aes(Longitude, Latitude), color="skyblue4", size=1, alpha=0.7)
ggsave(pl, file="./powassan_pts.png", device="png", units="in", width=10, height=7, dpi=600)
...
ggsave(plot_covars, file="./powv_covars.png", device="png", units="in", dpi=600, width=10, height=10)
...
dd[ , c(5:12) ] <- apply(dd[ , c(5:12) ] , 2, scale)

# save 
write.csv(dd, "./output/model_df/powassan_df.csv", row.names=FALSE)
...
m1i = fitINLAmodel(formx = formula("y ~ -1 + Intercept + f(s, model=spde) + agri_cover"))
m1j = fitINLAmodel(formx = formula("y ~ -1 + Intercept + f(s, model=spde) + urban_cover"))

# infocrit
exWAIC = function(m) return(m$waic$waic)
ic = data.frame(model=c("baseline",  "tmean_slope", "tmean_anomaly", "popdens_log", "healthtraveltime_log", "precip_anomaly", "precip_wetness", "agri_expansion", "urban_expansion", "agri_cover", "urban_cover"), 
                waic = sapply(list(m0, m1a, m1b, m1c, m1d, m1e, m1f, m1g, m1h, m1i, m1j), exWAIC))

# fit multivariate model 
m2 = fitINLAmodel(formx = formula("y ~ -1 + Intercept + f(s, model=spde) + tmean_slope + precip_anomaly + popdens_log + agri_cover + urban_cover + urban_expansion"))
ic = rbind(ic, data.frame(model="multivariate", waic = exWAIC(m2)))

# fit multivariate nonspatial 
m3 = fitINLAmodel(formx = formula("y ~ -1 + Intercept + tmean_slope + precip_anomaly + popdens_log + agri_cover + urban_cover + urban_expansion"))
...
spde_plot = ggplot() + 
  ...
  ggtitle("Powassan virus disease")
...
params_plot1 = rbind(fu, fm) %>%
  ...
  dplyr::mutate(type = factor(type, levels=c("Univariate (spatial)", "Multivariate (spatial)"), ordered=TRUE)) %>%
  ggplot() + 
