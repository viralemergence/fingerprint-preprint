...
# jitter lat-lons for fitting SPDE to polygons
locs_jit = data.frame()
for(i in 1:nrow(dz)){
  cat(i)
...
dz = cbind(dz, locs_jit %>% dplyr::rename("Longitude_jit"=X, "Latitude_jit"=Y))
...
dd = rbind(
  dz %>% dplyr::select(record_id, Longitude, Latitude, Longitude_jit, Latitude_jit, ADMcode) %>% dplyr::mutate(presence = 1),
  pbuf %>% dplyr::select(record_id, Longitude, Latitude, ADMcode) %>% dplyr::mutate(presence = 0, Longitude_jit=Longitude, Latitude_jit=Latitude) 
) %>%
...
ggsave(pdx, file="./output/plots/background_pt_maps/westnile_neuro_weighted_background_wp.png", device="png", units="in", dpi=900, width=9, height=5.5)
...
# save mpx df
write.csv(dd, "./output/model_df/westnileneuro_df_wp.csv", row.names=FALSE)
...
ggsave(plot_covars, file="./westnileneuro_covars.png", device="png", units="in", dpi=600, width=10, height=10)
...
dd = read.csv("./output/model_df/westnileneuro_df_wp.csv") %>%
...
usdm::vifstep(dd[ , 8:ncol(dd) ])
corrplot::corrplot(cor(dd[ , 8:ncol(dd) ]))
...
# jittered
dd = dd %>% dplyr::mutate(Longitude = Longitude_jit, Latitude=Latitude_jit)
...
plot(mesh1); points(spatial_locs, col="red", pch=16, cex=0.2)
...
form = formula(paste(c(form_base, predictors), collapse=" + "))
...
preds_c = c("tmean_anomaly", "precip_anomaly", "agri_expansion",
            "forest_cover", "forest_loss", "urban_cover", "agri_cover", "healthtraveltime_log", "social_vulnerability")
...
spde_plot = rasteriseSPDE(m2$summary.random$s$mean, mesh1, study_area) %>%
  ...
  maptheme +
  geom_sf(data=study_area, fill=NA, col="black") + 
  geom_point(data=dd[ dd$presence == 1, ], aes(Longitude, Latitude), col="red", size=0.2, alpha=0.2) + 
  theme(legend.position = c(0.98, 0.15), plot.title = element_text(hjust=0.5, size=16)) +
  ggtitle("West Nile virus (neuroinvasive)")
...
fx = do.call(rbind.data.frame, list(f1, f2, f3)) %>% 
  dplyr::mutate(Disease = "West Nile disease (neuroinvasive)",
                Num_observations = sum(dd$presence))
row.names(fx) = c()
write.csv(fx, "./output/model_outputs/disease_params/westnileneuro_params.csv", row.names=FALSE)
...
ggsave(pp, file="./output/plots/westnileneuro_dashboardtest_jun22.png", device="png", units="in", width=13, height=11, dpi=600, scale=0.9)
