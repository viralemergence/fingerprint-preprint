...
ggsave(plot_covars, file="./rvf_covars.png", device="png", units="in", dpi=600, width=10, height=10)

# save 
write.csv(dd, "./output/model_df/rvf_df_wp.csv", row.names=FALSE)
...
# data and keep complete cases (99.8%)
...
preds_c = c("tmean_anomaly", "precip_anomaly", 
            "agri_cover", "agri_expansion", "urban_cover",
            "healthtraveltime_log", "livestock_log")
...
row.names(fx) = c()
write.csv(fx, "./output/model_outputs/disease_params/rvf_params.csv", row.names=FALSE)
...
ggsave(pp, file="./output/plots/rvf_dashboardtest_jun22.png", device="png", units="in", width=13, height=11, dpi=600, scale=0.9)
