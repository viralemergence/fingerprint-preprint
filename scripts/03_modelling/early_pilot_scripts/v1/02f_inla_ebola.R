...
disease_name = "ebola"
dz = read.csv("./output/spillovers_processed/spillovers_ebola.csv") %>%
  distinct() %>%
  dplyr::filter(Year >= 1980) %>%
  dplyr::mutate(record_id = 1:length(Disease))
load("./output/spillovers_processed/spillovers_ebola_shp.R")
dz_shp = shp %>%
...
ggsave(pdx, file="./ebola_weighted_background_wp.png", device="png", units="in", dpi=900, width=9, height=5.5)
...
ggsave(plot_covars, file="./ebola_covars.png", device="png", units="in", dpi=600, width=10, height=10)

# save mpx df
write.csv(dd, "./output/model_df/ebola_df_wp.csv", row.names=FALSE)
...
# data and keep complete cases
dd = read.csv("./output/model_df/ebola_df_wp.csv") %>%
  dplyr::select(-popdens_log)
sum(complete.cases(dd)) / nrow(dd)
dd = dd[ complete.cases(dd), ]

# scale non log predictors
cols_for_scaling = c("forest_loss", "forest_cover", "tmean_anomaly", "agri_expansion", "agri_cover", "urban_expansion", "urban_cover", "precip_anomaly", "hunting_defaunation")
dd[ , which(names(dd) %in% cols_for_scaling) ] <- apply(dd[ , which(names(dd) %in% cols_for_scaling) ], 2, scale)

# # check for multicollinearity 
# urban cover vs urban expansion - remove UX
# forest cover vs agri cover - remove agri
...
predictors = c("tmean_anomaly", "precip_anomaly", 
               ...
               "healthtraveltime_log", "hunting_defaunation", "livestock_log")
...
form = formula(paste(c(form_base, predictors[-which(predictors == "forest_cover") ]), collapse=" + "))
m2 = fitINLAModel(formx=form, family=family1)

# multivariate model (causal subset)
preds_c = c("tmean_anomaly", "precip_anomaly", 
            "agri_cover", "forest_loss",
            "healthtraveltime_log", "hunting_defaunation")
...
spde_plot = rasteriseSPDE(m2$summary.random$s$mean, mesh1, study_area) %>%
  ...
  theme(legend.position = c(0.1, 0.15), plot.title = element_text(hjust=0.5, size=16)) +
  ggtitle("Ebola")
...
fx = do.call(rbind.data.frame, list(f1, f2, f3)) %>% 
  dplyr::mutate(Disease = "Ebola",
                Num_observations = sum(dd$presence))
row.names(fx) = c()
write.csv(fx, "./output/model_outputs/disease_params/ebola_params.csv", row.names=FALSE)
...
fx %>%
  ...
  dplyr::mutate(param = replace(param, param == "healthtraveltime_log", "Travel time to\nhealth facility (log)"),
                ...
                param = replace(param, param == "hunting_defaunation", "Hunting\npressure"),
...
ggsave(pp, file="./output/plots/ebola_dashboardtest_jun22.png", device="png", units="in", width=13.5, height=11, dpi=600, scale=0.9)
