...
# read disease data and reduce to single outbreak per year (distinct) post 1980
disease_name = "yfv"
...
# study area
sa = sf::st_read("data/shapefiles/ne_10m_land.shp")
study_area = st_union(sa)
...
sf::sf_use_s2(TRUE)
area_thresh = 25000 # equiv to ~55km buffer
...
ggsave(pdx, file="./yf_weighted_background_wp.png", device="png", units="in", dpi=600, width=8, height=8)
...
ggsave(plot_covars, file="./yfv_covars.png", device="png", units="in", dpi=600, width=10, height=10)

# save 
write.csv(dd, "./output/model_df/yfv_df_wp.csv", row.names=FALSE)
...
dd = read.csv("./output/model_df/yfv_df_wp.csv") %>%
...
preds_c = c("tmean_anomaly", "precip_anomaly", 
            "forest_cover", "forest_loss", 
            "agri_cover", "urban_cover",
            "healthtraveltime_log")
...
row.names(fx) = c()
write.csv(fx, "./output/model_outputs/disease_params/yfv_params.csv", row.names=FALSE)
...
ggsave(pp, file="./output/plots/yfv_dashboardtest_jun22.png", device="png", units="in", width=13.5, height=11, dpi=600, scale=0.9)
