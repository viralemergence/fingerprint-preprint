...
# ggsave(pdx, file="./output/plots/background_pt_maps/westnile_weighted_background_wp.png", device="png", units="in", dpi=900, width=9, height=5.5)
...
# save mpx df
write.csv(dd, "./output/model_df/westnile_df_wp.csv", row.names=FALSE)
...
ggsave(plot_covars, file="./westnile_covars.png", device="png", units="in", dpi=600, width=10, height=10)
...
dd = read.csv("./output/model_df/westnile_df_wp.csv") %>%
...
fx = do.call(rbind.data.frame, list(f1, f2, f3)) %>% 
  dplyr::mutate(Disease = "West Nile disease",
                Num_observations = sum(dd$presence))
row.names(fx) = c()
write.csv(fx, "./output/model_outputs/disease_params/westnile_params.csv", row.names=FALSE)
...
ggsave(pp, file="./output/plots/westnile_dashboardtest_jun22.png", device="png", units="in", width=13, height=11, dpi=600, scale=0.9)
