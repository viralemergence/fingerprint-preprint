...
mc_vars_to_exclude = c("urban_expansion", "social_vulnerability", "health_travel_log")
...
rasteriseSPDE(m3$summary.random$s$mean, mesh1, study_area) %>%
  ...
  geom_point(data=dd[ dd$presence == 1, ], aes(Longitude, Latitude), col="red", size=0.2, alpha=0.1) + 
