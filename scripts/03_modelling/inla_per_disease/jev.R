...
# read disease data
dz = read.csv("./output/spillovers_processed/spillovers_jev.csv") %>%
  dplyr::filter(Year >= 1985) %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease))
load("./output/spillovers_processed/spillovers_jev_shp.R")
dz_shp = shp1 %>%
...
num_background_points = round(nrow(dz) * 1.5) # number of background points to generate
...
if(generate_dataset == TRUE){
  ...
  # add pigs as amplifying host for JEV
  extr_x = extractCovariateVals(dd, covariate_name="livestock_pigs")
  ddx = cbind(ddx, extr_x)
  
  # log transform any relevant variables, drop geometry and save
  ddx = ddx %>%
    dplyr::mutate(health_travel_log = log(health_travel+1),
                  livestock_log = log(livestock_pigs+1))
...
mc_vars_to_exclude = c("urban_expansion", "forest_cover", "urban_expansion", "social_vulnerability")
