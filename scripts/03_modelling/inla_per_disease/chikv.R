...
dz = read.csv("./output/spillovers_processed/spillovers_chikv.csv") %>%
  dplyr::filter(Source == "Nsoesie2016") %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>%
  dplyr::filter(!is.na(Longitude)) %>%
  dplyr::filter(Year >= 1985) %>%
  dplyr::filter(!Country %in% c("American Samoa", "French Polynesia", "New Caledonia")) # n.b. 5 points removed due to data sparsity and absences issue
load("./output/spillovers_processed/spillovers_chikv_shp.R")
dz_shp = shp %>%
  dplyr::filter(ADMcode %in% dz$ADMcode)

# create study area polygon
study_area = createStudyAreaPolygon(lat=dz$Latitude, lon=dz$Longitude, buffer = TRUE, extent_border = 20, buf_size=300000)
...
if(generate_dataset == TRUE){
  ...
  bg = generateBackgroundPolygons(num_points = num_background_points,
                                  ...
                                  method = "popweight",
                                  seed = rseed,
                                  pop_res = "10km") # for dealing with such a large area
...
# library(mgcv)
# formx = paste(c("y ~ 1 + s(Longitude, Latitude, bs='gp', m=c(3, 20))", paste("s(", preds_c, ", k=5)", sep="")), collapse=" + ")
# formx = paste(c("y ~ 1 + s(Longitude, Latitude)", paste("s(", preds_c, ", k=5)", sep="")), collapse=" + ")
# 
# m4 = mgcv::gam(formula(formx), data=dd, family=binomial(link="logit"))
# summary(m4)
# gratia::draw(m4)
