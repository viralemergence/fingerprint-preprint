...
dz = read.csv("./output/spillovers_processed/spillovers_cchf.csv") %>%
  dplyr::filter(Year >= 1985) %>%
  dplyr::filter(!is.na(Longitude)) %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>%
  distinct() %>%
  dplyr::filter(Source != "Ak2020")

load("./output/spillovers_processed/spillovers_cchf_shp.R")
dz_shp = shp %>%
...
if(generate_dataset == TRUE){
  ...
  dz = dz[ !is.na(sf::st_dimension(dz)), ]
  ...
  # extract ruminant livestock as these are the main species involved in CCHF
  extr_x = extractCovariateVals(dd, covariate_name="livestock_ruminants")
  ddx = cbind(ddx, extr_x)
  
  # log transform any relevant variables, drop geometry and save
  ddx = ddx %>%
    dplyr::mutate(health_travel_log = log(health_travel+1),
                  livestock_log = log(livestock_ruminants+1))
...
dd = read.csv(filename) %>%
  dplyr::select(-health_travel, -livestock_ruminants, -evi_coefvar, -tmean, -precip, -tmean_anomaly, -precip_anomaly)
