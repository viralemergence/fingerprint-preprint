...
dz = read.csv("./output/spillovers_processed/spillovers_h5n1.csv") %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>%
  dplyr::filter(!is.na(Longitude))
dz = dz[ -which(dz$Country == "China" & dz$IfPolygon_AdminLevel == 1), ] # remove China provinces ahead of size filter for study area (massive)
dz = dz[ dz$LocationName != "Urumqi", ]
load("./output/spillovers_processed/spillovers_h5n1_shp.R")
...
hyp_lib = read.csv("./output/hypotheses/consensus_hypotheses_matched.csv") %>%
  ...
  dplyr::filter(dz_name == "Influenza A H5N1") %>%
...
dd = read.csv(filename) %>%
  dplyr::select(-health_travel, -livestock_all, -livestock_poultry, -evi_coefvar, -tmean, -precip, -tmean_anomaly, -precip_anomaly)
...
mc_vars_to_exclude = c("urban_expansion", "health_travel_log", "social_vulnerability", "biodiv_intact")

# examine VIFs/corplots for multivar and causal var subsets
# corrplot::corrplot(cor(dd[ , 6:ncol(dd) ] %>% dplyr::select(all_of(hyp_lib)) %>% dplyr::select(-all_of(mc_vars_to_exclude))  %>% dplyr::filter(complete.cases(.))))
