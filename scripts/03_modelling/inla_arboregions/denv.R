...
# ================ DENGUE (REGION-SPECIFIC SUBSETS) ===================
...
# ------------- split into datasets for Asia Pacific, Africa, Latin America --------------
...
dd_lac = rbind(
  dd_lac %>% dplyr::filter(presence == 0),
  dd_lac %>% dplyr::filter(presence == 1) %>% dplyr::sample_n(size=2000)
)
dd_asia = dd %>% dplyr::filter(Longitude >52 & Longitude < 170)
dd_africa = dd %>% dplyr::filter(Longitude <52 & Longitude > -28 & Latitude < 30)
dd_africa = rbind(
  dd_africa %>% dplyr::filter(presence == 1),
  dd_africa %>% dplyr::filter(presence == 0) %>% dplyr::sample_n(size=2000)
)
...
for(i in 1:3){
  ...
  if(i == 1){ mc_vars_to_exclude = c() }
  if(i == 2){ mc_vars_to_exclude = c("social_vulnerability") }
  if(i == 3){ mc_vars_to_exclude = c("urban_expansion") }
