...
dz = read.csv("./output/spillovers_processed/spillovers_yfv.csv") %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>%
  dplyr::filter(!is.na(Longitude)) %>%
  dplyr::filter(Year >= 1985)
load("./output/spillovers_processed/spillovers_yfv_shp.R")
dz_shp = shp %>%
...
num_background_points = nrow(dz) * 4 # number of background points to generate
...
hyp_cons = read.csv("./output/hypotheses/consensus_hypotheses_matched.csv") %>%
  dplyr::filter(covar_name != "") %>%
  dplyr::filter(dz_name == dz$Disease[1]) %>%
  dplyr::filter(TestAll == "Test") %>%
  dplyr::select(covar_name)
hyp_cons = as.vector(hyp_cons$covar_name)
...
if("forest_loss" %in% hyp_cons ){ hyp_cons = c(hyp_cons, "forest_cover") }
...
hyp_cons = unique(c(hyp_cons, confound)); hyp_cons = hyp_cons[ hyp_cons != "" ]
...
# ---------- fit models for Americas and Africa ------------
...
dd_lac = dd %>% dplyr::filter(Longitude < -28 & Longitude > -120 & Latitude < 33)
dd_africa = dd %>% dplyr::filter(Longitude <52 & Longitude > -28 & Latitude < 30)
...
for(i in 1:2){
  ...
  # create study area
  study_area = createStudyAreaPolygon(lat=dd$Latitude, lon=dd$Longitude, buffer=TRUE, extent_border = 8)
  ...
  corrplot::corrplot(cor(dd[ complete.cases(dd), 6:ncol(dd) ]))
  
  # specify which multicolinear vars to exclude in later analyses if any
  if(i == 1){ mc_vars_to_exclude = c("urban_expansion") }
  if(i == 2){ mc_vars_to_exclude = c("urban_expansion") }
  ...
  m_i = fitINLAModel(formx=form, family=family1, stack=stack1, spde=spde, config=TRUE)
