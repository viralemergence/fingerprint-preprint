...
dz = read.csv("./output/spillovers_processed/spillovers_chagas.csv") %>%
  dplyr::filter(Year >= 1985) %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>%
  dplyr::filter(!is.na(Longitude)) 
load("./output/spillovers_processed/spillovers_chagas_shp.R")
dz_shp = shp; rm(shp)

# filter out Browne Chagas records from Brazil (more informal and potential double counting with surv system)
dz = dz[ -which(dz$Source == "Browne2017" & dz$Country == "Brazil"), ]

# create study area polygon
study_area = createStudyAreaPolygon(lat=dz$Latitude, lon=dz$Longitude)

p1 = ggplot() + 
  geom_sf(data = study_area, fill="grey96", color="grey60") +
  geom_sf(data = dz_shp, fill=NA, color="darkred") + 
  geom_point(data = dz %>% dplyr::filter(DataType == "point"), aes(Longitude, Latitude), fill=NA, color="darkred", size=0.2) + 
  ggtitle("Chagas disease (acute)") +
  theme_void()  +
...
# random seed and disease name
rseed = 58937
disease_name = "oropouche"

# disease data and shapefile post-1985
dz = read.csv("./output/spillovers_processed/spillovers_oropouche.csv") %>%
  dplyr::filter(Year > 1980) %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease), 
                Disease = "Oropouche fever") 
load("./output/spillovers_processed/spillovers_oropouche_shp.R")
dz_shp = shp; rm(shp)
...
dz = read.csv("./output/spillovers_processed/spillovers_junin.csv") %>%
  ...
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease))
dz_shp = st_as_sf(dz, coords = c("Longitude", "Latitude"))
st_crs(dz_shp) = st_crs(study_area)
...
p2 = ggplot() + 
  geom_sf(data = study_area, fill="grey96", color="grey60") +
  geom_sf(data = dz_shp, fill=NA, color="darkred") + 
  ggtitle("Argentine haemorrhagic fever") +
  theme_void()  +
  theme(plot.title = element_text(size=15, hjust=0.5))
...
# disease data and shapefile post-1985
dz = read.csv("./output/spillovers_processed/spillovers_lyme.csv") %>%
  distinct() %>%
  dplyr::mutate(record_id = 1:length(Disease)) %>%
  dplyr::filter(!is.na(Longitude)) %>%
  dplyr::filter(Latitude < 50) # continental + non-alaska (contiguous US)
load("./output/spillovers_processed/spillovers_lyme_shp.R")
dz_shp = shp %>%
  dplyr::filter(ADMcode %in% dz$ADMcode)
rm(shp)

# create study area polygon (unique only USA)
#study_area = createStudyAreaPolygon(lat=dz$Latitude, lon=dz$Longitude)
...
# field for dz_shp
a = dz %>% dplyr::group_by(ADMcode) %>%
  dplyr::summarise(n_out = sum(NumCases > 0)) 
dz_shp = left_join(dz_shp, a)

p3 = ggplot() + 
  geom_sf(data = dz_shp, aes(fill=n_out), color=NA) +
  geom_sf(data = study_area, fill=NA, color="grey60") +
  scale_fill_gradientn(colors=colorRampPalette(c("white", "darkred"))(200), name="Num. outbreak years") +
  ggtitle("Lyme disease") +
  theme_void()  +
  theme(plot.title = element_text(size=15, hjust=0.5), legend.title = element_text(size=13), legend.position = "bottom")


pc = gridExtra::grid.arrange(p2, p1, p3, nrow=1)
ggsave(pc, file="./datatypes.jpg", device="jpg", units="in", width=18, height=6, dpi=600)

ggsave(p1, file="./chagas.jpg", device="jpg", units="in", width=5, height=5, dpi=600)
ggsave(p2, file="./junin.jpg", device="jpg", units="in", width=5, height=5, dpi=600)
ggsave(p3, file="./lyme.jpg", device="jpg", units="in", width=6, height=5, dpi=600)
ggsave(p4, file="./oropouche.jpg", device="jpg", units="in", width=6, height=5, dpi=600)
...
num_background_points = nrow(dz) * 2 # number of background points to generate
...
if(generate_dataset == TRUE){
  ...
  # remove incompatible sf types
  dz = dz[ st_geometry_type(dz) %in% c("POLYGON", "MULTIPOLYGON"), ]
...
# build mesh using spatial locs (requires some tuning per disease)
