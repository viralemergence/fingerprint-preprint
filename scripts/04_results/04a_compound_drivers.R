...
build_graph = function(param_results){
  ...
  nodes = left_join(nodes, read.csv("./scripts/04_results/driver_abbrevs.csv"), by=c("node"="driver")) %>%
    dplyr::mutate(
      ...
      abbrev2 = replace(abbrev2, abbrev2 == "LandHet", "Land\nhet"),
  ...
  node_order = c("Urban\nexp", "Urban", "Crop", "Crop\nexp", "Biodiv", "Health\naccess", "Hunting", "Mining", "Land\nhet", "Livestock", "Social\nvul", "Prot\narea", "Forest",  "Forest\nloss", "Precip\nchange", "Temp\nchange")
  nodes = nodes[ match(node_order, nodes$abbrev2), ]
  
  # driver type
  nodes$driver_type = "Land use intensity"
  nodes$driver_type[ nodes$node %in% c("Biodiversity Intactness", "Cropland cover", "Forest cover", "Landscape heterogeneity")] = "Ecosystem structure"
  ...
  nodes$driver_type = factor(nodes$driver_type, levels=c("Detection", "Socioeconomic", "Ecosystem structure", "Climate change", "Land use intensity"), ordered=TRUE)
...
for(i in 1:length(hyps)){
  ...
  pc = gridExtra::grid.arrange(pg1, pg2, pg3, ncol=3)
  ...
  filename = paste("./output/plots/Figure4_networks_", hn, ".jpg", sep="")
  ggsave(pc, file=filename, device="jpg", units="in", width=16.5, height=7.2, dpi=600, scale=0.8)
...
createNetwork = function(x, vifs){
  ...
  nodes$abbrev3[ nodes$abbrev3 == "Land\\nhet" ] = "Land.\nhet."
...
ggsave(comb,
       file="./output/plots/SuppFigure_DriverCorrelations_Revision.jpg",
